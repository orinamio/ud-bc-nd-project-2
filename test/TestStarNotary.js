...
it("can add the star name and star symbol properly", async () => {
	...
	assert(instance.name.call(), "O Crypto Star");
	assert(instance.symbol.call(), "OCRS");
});
...
it("lookUptokenIdToStarInfo test", async () => {
	...
	const instance = await StarNotary.deployed();
	const starId = 10;
	const starName = "Star 10";
	const user = accounts[1];
	await instance.createStar(starName, starId, { from: user });

	assert(await instance.lookUptokenIdToStarInfo.call(starId), starName);
});
